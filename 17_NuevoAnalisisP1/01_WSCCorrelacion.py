...
def compute_policy_metrics(rules, dataset):
    ...
    metrics = {
        ...
        "access_mean": np.mean(access),
        #"access_std": np.std(access),
        #"access_max": np.max(access),
        #"subsumption_total": np.sum(subsumed),
        "subset_ratio": np.mean(subsumed)
    }

    return metrics

def compute_wsc_correlations(df):

    metrics = [c for c in df.columns if c not in ["dataset", "wsc"]]

    rows = []

    for col in metrics:

        pearson = df["wsc"].corr(df[col], method="pearson")
        spearman = df["wsc"].corr(df[col], method="spearman")
    ...
    print("\n ----- Correlacion WSC vs Otras Metricas ----- ")
    print(corr_df.to_string(index=False))

    corr_df.to_csv("wsc_correlations.csv", index=False)

    return corr_df

def compute_correlation_matrix(df):

    numeric_df = df.drop(columns=["dataset"])
    ...
    pearson_matrix.to_csv("correlation_matrix.csv")
...
def multi_dataset_experiment(configs):

    results = []

    for name, rules_path, dataset_path in configs:

        rules = load_rules(rules_path)
        dataset = load_dataset(dataset_path)

        metrics = compute_policy_metrics(rules, dataset)
    ...
    df.to_csv("all_policy_metrics.csv", index=False)

    print("\n ----- Todas las metricas -----")
    print(df.to_string(index=False))

    compute_wsc_correlations(df)
...
if __name__ == "__main__":
    ...
    configs = [
    ("AMZ", "reglas_abac.txt", "train.csv"),
    ("HC", "reglas_abac-hc.txt", "0-HC-universal.csv"),
    ("IoT", "reglas_abac_iot.txt", "IoT-Rw.csv"),
    ("KAMZ", "reglas_abac_karimi.txt", "train.csv"),
    ("KHC", "reglas_abac_karimi_hc.txt", "0-HC-universal.csv"),
    ("KIoT", "reglas_abac_karimi_iot.txt", "IoT-Rw.csv"),
]

multi_dataset_experiment(configs)