...
class PolicyMining:
    ...
    def data_preprocessing(self, is_noisy=False, is_sparse=False):
        ...
        if self.name_ds == 'AMZ':  # AMZ Dataset
            ...
            self.df_data = self.df_data[self.df_data.columns[1:]].drop_duplicates()
            #self.df_data = self.df_data.sample(frac=0.2, random_state=42)
    ...
    def rule_inference(self, th_rule_sim, th_lfr = 0.2, th_sr = 0.5):
        print("\n##########################")
    ...
    def rule_inference_hc(self, th_rule_sim, th_lfr=0.2, th_sr=0.5):
        ...
        for commty_ in self.all_commts:
            ...
            # Copias temporales para ir eliminando registros cubiertos
            df_res_remaining = df_res_commty.copy()
            df_users_remaining = df_users_commty.copy()

            generated_rules = 0

            while generated_rules < 15:

                # Si ya no hay datos suficientes, detener
                if len(df_res_remaining) == 0 or len(df_users_remaining) == 0:
                    break
                ...
                # atributos frecuentes recursos
                rule_res_attrs = attribute_value_common(df_res_remaining)

                # atributos frecuentes usuarios
                rule_user_attrs = attribute_value_common(df_users_remaining)
                ...
                # =========================
                # ELIMINAR REGISTROS CUBIERTOS
                # =========================

                # Recursos
                for attr, value in rule_res_attrs:

                    df_res_remaining = df_res_remaining[
                        df_res_remaining[attr] != value
                    ]

                # Usuarios
                for attr, value in rule_user_attrs:

                    df_users_remaining = df_users_remaining[
                        df_users_remaining[attr] != value
                    ]

                generated_rules += 1
                print(generated_rules, len(df_res_remaining))
    ...
    def experiment_bridge_node_reassignment(self,
        ...
        for it in range(iterations):
            ...
            # eliminar comunidades sin nodos
            unique_comms = sorted(set(new_membership))
            mapper = {old: new for new, old in enumerate(unique_comms)}
            new_membership = [mapper[c] for c in new_membership]
            g.vs["commty"] = new_membership
            self.all_commts = list(sorted(set(new_membership)))
    ...
    def compute_rule_coverage(self, rule):
        """
        Returns the number of log records covered by a rule.
        """
        df_temp = self.df_data

        for attr, val in rule[1]:
            if attr in df_temp.columns:
                df_temp = df_temp[df_temp[attr] == val]

        return len(df_temp)
    
    
    def compute_rules_coverage(self, rules):
        """
        Computes coverage for each rule and returns a list with coverage values.
        """
        rules_cov = []

        for idx, rule in enumerate(rules):
            cov = self.compute_rule_coverage(rule)
            rules_cov.append({
                "rule_id": idx,
                "rule": rule,
                "coverage": cov
    ...
    def save_rules_coverage(self, rules_cov, output_path="rule_coverage.csv"):
        """
        Save rule coverage information to a CSV file.
        """
        ...
        print("Coverage file saved at:", output_path)

        return df_cov
    
    def plot_coverage_distribution(self, df_cov, output_path="coverage_per_rule.png"):
        """
        Plot rule coverage where:
        X axis = rule id
        Y axis = number of records covered
        """

        plt.figure(figsize=(10,5))

        plt.bar(df_cov["rule_id"], sorted(df_cov["coverage"]))

        plt.xlabel("Rule ID")
        plt.ylabel("Number of Covered Records")
        ...
        print("Coverage plot saved at:", output_path)
...
# ----------------------------
# Experiment: Bridge node reassignment (integrated as a function and bound to PolicyMining)
# ----------------------------
import random
import copy
...
def _rm_jaccard_index(set1, set2):
    intersection = len(set1 & set2)
    union = len(set1 | set2)
    return intersection / union if union != 0 else 0.0

def _rm_run_louvain_iterations(g, iterations=10):
    same_community_neighbors = defaultdict(list)
    for _ in range(iterations):
        partition = g.community_multilevel()
        membership = partition.membership
        node_to_comm = {v.index: membership[v.index] for v in g.vs}
        for v in g.vs:
            node_id = v['id'] if 'id' in v.attribute_names() else v.index
            community_neighbors = set()
            for neighbor in g.neighbors(v):
                if node_to_comm[neighbor] == node_to_comm[v.index]:
                    nb_id = g.vs[neighbor]['id'] if 'id' in g.vs[neighbor].attribute_names() else g.vs[neighbor].index
                    community_neighbors.add(nb_id)
            same_community_neighbors[node_id].append(community_neighbors)
    return same_community_neighbors

def _rm_compute_jaccard_stability(same_community_neighbors):
    stability_scores = {}
    for node_id, neighbor_sets in same_community_neighbors.items():
        if len(neighbor_sets) < 2:
            stability_scores[node_id] = 1.0
        ...
        jaccard_scores = []
        for s1, s2 in combinations(neighbor_sets, 2):
            jaccard_scores.append(_rm_jaccard_index(s1, s2))
        stability_scores[node_id] = sum(jaccard_scores) / len(jaccard_scores)
    return stability_scores

def _canonical_rule_set(rule, ignore_comm=False):
    # rule: [["id_com", "X"], [[attr,val],...]]
    header, body = rule[0], rule[1]
    pairs = []
    for a, v in body:
        pairs.append((str(a), str(v)))
    if not ignore_comm:
        # include id_com value
        pairs.append(("id_com", str(header[1])))
...
def experiment_bridge_node_reassignment(self,
                                       ratio_bridge_nodes=0.2,
                                       trials=10,
                                       stability_threshold=0.4,
                                       louvain_iterations=10,
                                       sample_seed=None,
                                       verbose=True):
    """Experimento externo que reasigna nodos puente (creando un 'commty_modified')
    y recalcula reglas y métricas para comparar con el baseline.

    - ratio_bridge_nodes: fracción de nodos puente a perturbar en cada trial.
    - trials: cuántas repeticiones aleatorias realizar.
    - stability_threshold: Jaccard de estabilidad bajo la cual un nodo se considera puente.
    - louvain_iterations: repeticiones de Louvain para estimar estabilidad.
    - sample_seed: semilla para reproducibilidad.
    - verbose: si True imprime logs.
    Retorna un diccionario resumen con resultados por trial.
    """
    ...
    # 0) guardados previos
    if verbose: print("[EXP] Guardando estado original...")
    g = self.user_network
    # backup attributes
    original_commtys = copy.deepcopy(g.vs['commty']) if 'commty' in g.vs.attribute_names() else None
    original_all_commts = copy.deepcopy(getattr(self, 'all_commts', None))
    original_list_rules = copy.deepcopy(getattr(self, 'list_rules', None))
    original_rule_network = copy.deepcopy(getattr(self, 'rule_network', None))
    original_rules_with_idx = copy.deepcopy(getattr(self, 'rules_with_idx', None))
    original_fp = copy.deepcopy(getattr(self, 'fp_logs', None))
    original_fn = copy.deepcopy(getattr(self, 'fn_logs', None))

    # 1) Baseline: si no hay reglas/evaluacion, obtenerlas
    if verbose: print("[EXP] Asegurando baseline (rule_inference + evaluation)...")
    if getattr(self, 'list_rules', None) is None or len(self.list_rules) == 0:
        # assume community_detection already run and self.all_commts exists
        self.list_rules = []
        self.rule_inference(1)  # use default sim threshold 1 (original code may expect param)
        self.evaluation()
    else:
        # ensure evaluation computed
        if not hasattr(self, 'fp_logs') or not hasattr(self, 'fn_logs'):
            self.evaluation()

    baseline_rules = copy.deepcopy(self.list_rules)
    baseline_rules_set_inc = set(_canonical_rule_set(r, ignore_comm=False) for r in baseline_rules)
    baseline_rules_set_exc = set(_canonical_rule_set(r, ignore_comm=True) for r in baseline_rules)
    baseline_metrics = {
        'FN': len(getattr(self, 'fn_logs', [])),
        'FP': len(getattr(self, 'fp_logs', [])),
        'Precision': None, 'Recall': None, 'Fscore': None, 'WSC': None, '#Rules': len(baseline_rules)
    }
    try:
        TP = len(self.df_data_pos) - len(self.fn_logs)
        precision = TP / (TP + len(self.fp_logs)) if (TP + len(self.fp_logs))>0 else 0.0
        recall = TP / (TP + len(self.fn_logs)) if (TP + len(self.fn_logs))>0 else 0.0
        fscore = 2*(precision*recall)/(precision+recall) if (precision+recall)>0 else 0.0
        wsc = sum([len(rule[1]) for rule in baseline_rules])
        baseline_metrics.update({'Precision': precision, 'Recall': recall, 'Fscore': fscore, 'WSC': wsc})
    except Exception:
        pass

    if verbose:
        print(f"[EXP] Baseline: |rules|={len(baseline_rules)}, FN={baseline_metrics['FN']}, FP={baseline_metrics['FP']}")
    # 2) identificar nodos puente
    if verbose: print("[EXP] Corriendo Louvain multiple para estimar estabilidad...")
    neighs = _rm_run_louvain_iterations(g, iterations=louvain_iterations)
    stability = _rm_compute_jaccard_stability(neighs)
    # bridge nodes: those with stability <= threshold
    bridge_nodes = [nid for nid, s in stability.items() if s <= stability_threshold]
    if verbose: print(f"[EXP] Nodos puente detectados (stability <= {stability_threshold}): {len(bridge_nodes)}")

    if len(bridge_nodes) == 0:
        if verbose: print("[EXP] No hay nodos puente. Se restaura estado y se devuelve baseline.")
        # restore
        if original_commtys is not None:
            g.vs['commty'] = original_commtys
        return {'status':'no_bridge_nodes','baseline_metrics':baseline_metrics}

    # mapping external id -> index
    id_to_idx = {}
    for v in g.vs:
        node_id = v['id'] if 'id' in v.attribute_names() else v.index
        id_to_idx[node_id] = v.index

    # get unique communities from current commty
    current_comm_assignments = copy.deepcopy(g.vs['commty'])
    unique_comms = sorted(list(set(current_comm_assignments)))

    trial_results = []
    for t in range(trials):
        if verbose: print(f"\n[EXP] Trial {t+1}/{trials}")
        # start from original assignments
        modified_assignments = copy.deepcopy(current_comm_assignments)
        n_bridge = len(bridge_nodes)
        n_sample = max(1, int(round(n_bridge * ratio_bridge_nodes)))
        sampled = random.sample(bridge_nodes, n_sample)
        if verbose: print(f"[EXP] Sampled {n_sample} bridge nodes: {sampled}")
        # reassign sampled nodes randomly
        for ext_id in sampled:
            if ext_id not in id_to_idx:
                if verbose: print(f"[EXP] Warning: external id {ext_id} not found in graph; skip.")
                continue
            idx = id_to_idx[ext_id]
            cur = modified_assignments[idx]
            options = [c for c in unique_comms if c != cur]
            if not options:
                continue
            newc = random.choice(options)
            modified_assignments[idx] = newc
            if verbose: print(f"   - Nodo {ext_id}: {cur} -> {newc}")

        # store modified assignment as separate attribute (option B)
        g.vs['commty_modified'] = modified_assignments

        # For downstream functions, temporarily set 'commty' to modified, run sub_community_detection and add_type_commts
        backup_commty = copy.deepcopy(g.vs['commty'])
        g.vs['commty'] = modified_assignments

        # Recompute dict_commts and types using existing functions
        dict_commts_mod = sub_community_detection(g, 0.5, None)
        # compute thresholds as original method does
        try:
            n_res_in_comms = [len(i[1]) for i in dict_commts_mod.values()]
            max_n_res = max(n_res_in_comms) if n_res_in_comms else 0
            big_threshold = int(0.5 * max_n_res)
            med_threshold = int(0.25 * max_n_res)
        except Exception:
            big_threshold, med_threshold = 0, 0

        s_commts, m_commts, c_commts = add_type_commts(g, dict_commts_mod, big_threshold, med_threshold)
        all_commts_mod = s_commts + m_commts + c_commts

        # attach modified all_commts to self (temporarily)
        prev_all_commts = copy.deepcopy(getattr(self, 'all_commts', None))
        self.all_commts = all_commts_mod

        # run rule_inference and evaluation on modified communities
        # we'll backup list_rules etc.
        prev_list_rules = copy.deepcopy(getattr(self, 'list_rules', None))
        prev_rule_network = copy.deepcopy(getattr(self, 'rule_network', None))
        prev_rules_with_idx = copy.deepcopy(getattr(self, 'rules_with_idx', None))
        prev_fp = copy.deepcopy(getattr(self, 'fp_logs', None))
        prev_fn = copy.deepcopy(getattr(self, 'fn_logs', None))

        # Run
        if verbose: print('[EXP] Ejecutando rule_inference() sobre comunidades modificadas...')
        # call rule_inference; it doesn't take params so it will use self.all_commts
        self.list_rules = []
        self.rule_inference(1)  # use same th_rule_sim default used elsewhere
        self.evaluation()

        pert_rules = copy.deepcopy(self.list_rules)
        pert_rules_set_inc = set(_canonical_rule_set(r, ignore_comm=False) for r in pert_rules)
        pert_rules_set_exc = set(_canonical_rule_set(r, ignore_comm=True) for r in pert_rules)

        # compute jaccard similarity
        j_inc = _jaccard_sets(baseline_rules_set_inc, pert_rules_set_inc)
        j_exc = _jaccard_sets(baseline_rules_set_exc, pert_rules_set_exc)
        ...
        try:
            ...
            fscore = 2*(precision*recall)/(precision+recall) if (precision+recall)>0 else 0.0
            wsc = sum([len(rule[1]) for rule in self.list_rules])
            pert_metrics.update({'Precision': precision, 'Recall': recall, 'Fscore': fscore, 'WSC': wsc, '#Rules': len(self.list_rules)})
        except Exception:
            pass

        trial_results.append({
            'trial': t,
            'n_bridge_total': n_bridge,
            'n_sampled': n_sample,
            'sampled_ids': sampled,
            'jaccard_including_id': j_inc,
            'jaccard_excluding_id': j_exc,
            'baseline_metrics': baseline_metrics,
            'perturbed_metrics': pert_metrics
        })

        # restore previous lists/objects to avoid accumulating changes
        self.list_rules = prev_list_rules
        self.rule_network = prev_rule_network
        self.rules_with_idx = prev_rules_with_idx
        self.fp_logs = prev_fp
        self.fn_logs = prev_fn
        self.all_commts = prev_all_commts

        # restore commty attribute to original (backup_commty)
        g.vs['commty'] = backup_commty

    # Restore original topology attributes
    if original_commtys is not None:
        g.vs['commty'] = original_commtys
    self.list_rules = original_list_rules
    self.rule_network = original_rule_network
    self.rules_with_idx = original_rules_with_idx
    self.fp_logs = original_fp
    self.fn_logs = original_fn
    self.all_commts = original_all_commts

    # summary
    j_inc_list = [r['jaccard_including_id'] for r in trial_results]
    j_exc_list = [r['jaccard_excluding_id'] for r in trial_results]
    summary = {
        'trials': trials,
        'bridge_nodes_detected': len(bridge_nodes),
        'ratio_bridge_nodes': ratio_bridge_nodes,
        'stability_threshold': stability_threshold,
        'jaccard_including_mean': sum(j_inc_list)/len(j_inc_list) if j_inc_list else None,
        'jaccard_excluding_mean': sum(j_exc_list)/len(j_exc_list) if j_exc_list else None,
        'trial_results': trial_results,
        'baseline_metrics': baseline_metrics
    }

    if verbose:
        print('\n[EXP] Experimento completado. Resumen:')
        print(f"   - Trials: {summary['trials']}")
        print(f"   - Bridge nodes detected: {summary['bridge_nodes_detected']}")
        print(f"   - Jaccard (including id) mean: {summary['jaccard_including_mean']}")
